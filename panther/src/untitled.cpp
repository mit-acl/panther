...
void SolverGurobi::toGradDiffConstraintsDiffVariables(int var_gindex, const Eigen::Vector3d &tmp, double *grad, int r,
                                                      int nn)
...
void SolverGurobi::toGradSameConstraintDiffVariables(int var_gindex, const Eigen::Vector3d &tmp, double *grad, int r,
                                                     int nn)
...

{
  ...
  for (int i = 0; i <= N_; i++)
  {
    ...
    Eigen::Matrix<GRBVar, 3, 1> q_i;
    for (int j = 0; j < 3; j++)  // x,y,z
    {
      q_i(j, 1) = m.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_CONTINUOUS, "q_" + std::to_string(i) + coords[j]);
    }
...
void SolverGurobi::setObjective()
{
  ...
  for (int i = 0; i < num_of_segments_; i++)
  {
    Eigen::Matrix<double, 4, 1> A_i_times_tmp = A_pos_bs_[i] * tmp;  // TODO (this is 4x1)

    // std::vector<std::vector<GRBVar>> Q;
    // Q.push_back(q_var_[i]);
    // Q.push_back(q_var_[i + 1]);
    // Q.push_back(q_var_[i + 2]);
    // Q.push_back(q_var_[i + 3]);

    Eigen::Matrix<GRBVar, 3, 4> Q;
    Q.col(0) << q_var_[i].x(), q_var_[i].y(), q_var_[i].z();
    Q.col(1) << q_var_[i + 1].x(), q_var_[i + 1].y(), q_var_[i + 1].z();
    Q.col(2) << q_var_[i + 2].x(), q_var_[i + 2].y(), q_var_[i + 2].z();
    Q.col(3) << q_var_[i + 3].x(), q_var_[i + 3].y(), q_var_[i + 3].z();

    Eigen::Matrix<GRBLinExpr, -1, -1> tmp = myMultVD(Q, A_i_times_tmp);
    // std::vector<GRBLinExpr> tmp = matrixMultiply(Q, eigenVector2std(A_i_times_tmp));
    cost += getNorm2(tmp);
  }

  // cost += weight_modified_ * getNorm2(q_var_[N_]);
  ...
  // // Eigen::Matrix<GRBVar, 2, 1> b;
  // Eigen::Matrix<GRBVar, -1, -1> b(2, 1);
  // b << q_var_[0][0], q_var_[0][0];
...
//////////////////////////////////////////////////////////
template <class T>
void SolverGurobi::printInfeasibleConstraints(const T &constraints)
{
...
template <class T>
bool SolverGurobi::areTheseConstraintsFeasible(const T &constraints)
{
  for (int i = 0; i < num_of_constraints_; i++)
  {
    if (constraints[i] > epsilon_tol_constraints_)  // constraint is not satisfied yet
...
template <class T>
int SolverGurobi::getNumberOfInfeasibleConstraints(const T &constraints)
{
  int result = 0;
...
template <class T>
bool SolverGurobi::isFeasible(const T x)
{
  std::vector<Eigen::Vector3d> q;
  std::vector<Eigen::Vector3d> n;
  std::vector<double> d;
...
bool SolverGurobi::isFeasible(const std::vector<Eigen::Vector3d> &q, const std::vector<Eigen::Vector3d> &n,
...
/*template <class T>
void SolverGurobi::printInfeasibleConstraints(const T x)
{
  std::vector<Eigen::Vector3d> q;
  std::vector<Eigen::Vector3d> n;
  std::vector<double> d;
...
void SolverGurobi::printInfeasibleConstraints(std::vector<Eigen::Vector3d> &q, std::vector<Eigen::Vector3d> &n,
                                              std::vector<double> &d)
{
  double constraints[num_of_constraints_];
  computeConstraints(0, constraints, num_of_variables_, nullptr, q, n, d);
...
void SolverGurobi::qnd2x(const std::vector<Eigen::Vector3d> &q, const std::vector<Eigen::Vector3d> &n,
...
void SolverGurobi::initializeNumOfConstraints()
{
...
bool SolverGurobi::isDegenerate(const std::vector<double> &x)
{
...
template <class T>
void SolverGurobi::x2qnd(T &x, std::vector<Eigen::Vector3d> &q, std::vector<Eigen::Vector3d> &n, std::vector<double> &d)
{
  q.clear();
  n.clear();
  d.clear();
...
int SolverGurobi::gIndexQ(int i)
{
  return 3 * i - 9;  // Q0, Q1, Q2 are always fixed (not decision variables)
}

// global index of the first element of the normal i
int SolverGurobi::gIndexN(int i)
{
  return 3 * i + j_min_;
}

int SolverGurobi::gIndexD(int i)
{
  return i + k_min_;
}

void SolverGurobi::assignValueToGradConstraints(int var_gindex, const double &tmp, double *grad, int r, int nn)
...
// double SolverGurobi::computeObjFunction(unsigned nn, double *grad, std::vector<Eigen::Vector3d> &q,
//                                        std::vector<Eigen::Vector3d> &n, std::vector<double> &d)
// {
//   // Cost
//   double cost = 0.0;
...
double SolverGurobi::myObjFunc(unsigned nn, const double *x, double *grad, void *my_func_data)
{
  SolverGurobi *opt = reinterpret_cast<SolverGurobi *>(my_func_data);
...
bool SolverGurobi::checkGradientsUsingFiniteDiff()
{
...
// nn is the number of variables
void SolverGurobi::myIneqConstraints(unsigned m, double *constraints, unsigned nn, const double *x, double *grad,
                                     void *f_data)
{
...
nlopt::algorithm SolverGurobi::getSolver(std::string &solver)
{
...
std::string SolverGurobi::getResultCode(int &result)
{
  switch (result)
  {
    case -5:
