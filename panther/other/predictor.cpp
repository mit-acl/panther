...
Predictor::Predictor(ros::NodeHandle nh) : nh_(nh)
{
  ...
  pub_predicted_traj_ = nh_.advertise<panther_msgs::PieceWisePolTraj>("/trajs", 1);
  pub_marker_predicted_traj_ = nh_.advertise<visualization_msgs::MarkerArray>("marker_predicted_traj", 1);

  safeGetParam(nh_, "panther/Ra", R_local_map_);
...
void Predictor::trajCB(const panther_msgs::DynTraj& msg)
{
  if (msg.id == id_)
