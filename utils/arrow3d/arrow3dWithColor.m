function varargout=arrow3dWithColor(start,stop,ang,ltype,p,color,n)
%   Plot 3d Arrows
% 
...
if strcmp(ltype,'line')
    ...
    for i=1:length(dis)
        ...
        hlines(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',color);
...
for i=1:length(dis)
    ...
    [rx,ry,rz] = rotatedata(xi,yi,zi,nvec(i,:),cosrang(i),[0,0,0]);
    cx=start(i,1)+rx;cy=start(i,2)+ry;cz=start(i,3)+rz;
    hheads(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',color);
